...
int main() {
    ...
    if (qtdDeFuncionarios == 0) {
        printf("\nO arquivo nÃ£o contÃ©m registros vÃ¡lidos.\n");
        free(funcionarios);
        return 1;
    }
    ...
    do {
        ...
        if (scanf("%d", &opcao) != 1) {
            printf("Entrada invÃ¡lida. Por favor, digite um nÃºmero.\n");
            flushEntrada();
            continue;
        }
        if (opcao >= 1 && opcao <= 3) {
...
Funcionario* lerArquivo(const char *nomeArquivo, int *qtdDeFuncionarios) {
    ...
    while (fgets(buffer, sizeof(buffer), entrada)) {
        ...
        if (sscanf(buffer, " %99[^,], %c ,%d", f.nome, &f.setor, &f.idade) != 3) {
            fprintf(stderr, "Formato invÃ¡lido na linha %d: %s", linha, buffer);
            continue;
        }
...
// ============ IMPRIME FUNCIONÃRIOS ============
void imprimirFuncionarios(Funcionario *v, int qtd) {
    printf("\n=========================================================\n");
    printf("%d registros vÃ¡lidos lidos e ordenados:\n", qtd);
    for (int i = 0; i < qtd; i++) {
        printf("-----------------------------------------\n");
